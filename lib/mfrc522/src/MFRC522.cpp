 ...
 * Fetch the ATS.
 */
MFRC522::StatusCode MFRC522::PICC_ATS(
		byte* atsBuf,	///< [out] The ATS data (40 bytes)
		byte *len			///< [in,out] Buffer length (must be 16), returns ATS length
) {
	MFRC522::StatusCode result;
	byte buffer[4];
	...
	buffer[1] = 0x30; // 40 byte return, assign ID 0 to this card
	// Calculate CRC_A
	...
	result = PCD_TransceiveData(buffer, sizeof(buffer), atsBuf, len);
	return result;
} // End PICC_ATS()
 ...
 * Fetch the ATR.
 */
 MFRC522::StatusCode MFRC522::PICC_ATR(
		byte *atrBuf,	///< [out] The ATS data (45 bytes)
		byte *len			///< [in,out] Buffer length (must be 45), returns ATR length
) {
	MFRC522::StatusCode result;
	byte atsLen = *len - 5;
	result = PICC_ATS(atrBuf + 4, &atsLen);
	if (result != STATUS_OK)
		return result;

	// Add 5 bytes for the ATR data, then remove the CRC (2 bytes)
	*len = atsLen + 3;
	atrBuf[0] = 0x3B;
	atrBuf[1] = 0x80 + atrBuf[4];
	atrBuf[2] = 0x80;
	atrBuf[3] = 0x01;
	atrBuf[*len - 1] = 0;
	for (int i = 1; i < *len - 1; i++)
		atrBuf[*len - 1] ^= atrBuf[i];
